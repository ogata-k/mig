...
use file_helper::get_output_file_absolute_path;
...
// TODO このままではpanicで失敗が帰ってしまうので適宜returnするように変更する。
pub fn action_controller(matches: ArgMatches) -> Result<&str, &str> {
    ...
    if input_file_opt.is_none() { return Err("input file is not specified. Why?"); }
    ...
    if framework_opt.is_none() { return Err("target frame work is not specified. Why?"); }
    // framework_opt is not none!!
    let framework_type = to_framework_type(framework_opt.unwrap());
    if framework_type.is_none() {
        return
            Err("the framework is not supported.")
    }

    // TODO get output file path by using framework type
    let output_file_ = match matches.value_of("OUTPUT") {
        Some(file) => file,
        None => "",
    };
    let output_file_path = get_output_file_absolute_path(output_file_, &framework_type.unwrap().clone());

    // TODO exist? these files -> NO! is Ok, else this app failed
